...
namespace PuntoVenta.Modal
{
    ...
    public partial class mdVenta : Form
    {
        ...
        private void mdVenta_Load(object sender, EventArgs e)
        {
            ...
            List<Venta> list = new CN_Venta().listar();
        ...
        private void dgvUserData_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (filaSeleccionada >= 0 && columnaSelecionada >= 0)
            {
                _Venta = new Venta()
                {
                    idVenta = Convert.ToInt32(dgvUserData.Rows[filaSeleccionada].Cells["idVenta"].Value.ToString()),
                    tipoDocumento = dgvUserData.Rows[filaSeleccionada].Cells["tipoDocumento"].Value.ToString(),
                    numeroDocumento = dgvUserData.Rows[filaSeleccionada].Cells["documento"].Value.ToString(),
